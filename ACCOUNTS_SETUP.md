# Guía de configuración de cuentas — ThreadPulse
...
ThreadPulse consulta la **API pública de Algolia** que usa el propio Hacker News:
...
**Sí.** Sin ellas, ThreadPulse sigue funcionando en HN pero Reddit queda deshabilitado.
...
| 5 | **name:** `ThreadPulse` (o el nombre que quieras). |
...
| 10 | **User agent:** Reddit lo exige con formato legible, por ejemplo: `ThreadPulse/1.0 (by /u/tu_usuario_reddit)` |
...
- ThreadPulse busca en **subreddits configurados** (máx. 10 por keyword) con ventana de **24 horas**.
- **Recomendaciones:**
...
**Sí.** Variable `GETXAPI_API_KEY`.

ThreadPulse usa [GetXAPI](https://www.getxapi.com/) en lugar de la API oficial de X — sin cuenta de desarrollador ni mínimo de USD 100/mes.

### Paso a paso

| Paso | Acción |
|------|--------|
| 1 | Creá cuenta en [getxapi.com](https://www.getxapi.com/). |
| 2 | En el dashboard, copiá tu **API Key**. |
| 3 | Pegala en `GETXAPI_API_KEY` en `.env.local` y Vercel. |
| 4 | (Opcional) Ajustá `CRON_TWITTER_LOOKBACK_HOURS` — default `24`. |
...
| Costo | ~USD 0.001 por llamada (~20 tweets) |
| Operadores | `lang:en`, `-filter:retweets`, `since:YYYY-MM-DD`, etc. |
| Paginación | `cursor` + `next_cursor` |

### Cómo maneja ThreadPulse estos límites

- Busca hasta **~20 tweets** por keyword por corrida (máx. 3 páginas si hace falta).
- Ventana configurable con `CRON_TWITTER_LOOKBACK_HOURS` (default **24 h**).
- Si GetXAPI devuelve error, el cron lo registra y sigue con otras plataformas.

### Verificación

Con `GETXAPI_API_KEY` definido y una keyword con plataforma Twitter activa, corré el cron y buscá señales con badge **X**.
...
ThreadPulse usa la **misma búsqueda pública de Algolia** que el sitio de Indie Hackers (clave *search-only* expuesta en su frontend). **No hace scraping HTML.**
...
| `GETXAPI_API_KEY` | Opcional | [getxapi.com](https://www.getxapi.com/) → API Key |
| `CRON_TWITTER_LOOKBACK_HOURS` | Opcional | Ventana de búsqueda Twitter (default `24`) |
...
- [ ] Twitter: `GETXAPI_API_KEY` y señales **X**.
