...
import { autoGenerateDraftForNewSignal } from "@/lib/drafts/auto-draft";
...
async function processPostForKeyword(
  ...
  const draftResult = await autoGenerateDraftForNewSignal({
    signalId: signal.id,
  ...
  if (!draftResult.generated) {
    ...
    if (draftResult.error) {
      result.errors.push(`Draft falló (${post.externalId}): ${draftResult.error}`);
    }
  }
