...
function buildDigestHtml(
  ...
  <div style="font-family:Arial,sans-serif;background:#0A0A0A;color:#fff;padding:24px;">
    <h1 style="color:#34D399;">SubSignal</h1>
    ...
    <p style="font-size:12px;color:#6B6B6B;">© 2026 SubSignal — ${email}</p>
  </div>`;
}
  ...
  for (const user of users ?? []) {
    ...
    try {
      await resendEmailProvider.send({
        to: user.email,
        subject: `📊 Tu semana en SubSignal — ${totalSignals} señales encontradas`,
        ...
        text: `Tu semana en SubSignal: ${totalSignals} señales encontradas.`,
      });
      result.sent += 1;
    } catch (error) {
      result.errors.push(
